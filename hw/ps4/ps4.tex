...
$\theta$). 
%\footnote{We are using the book's definition of the Multinomial
%distribution, given on page 20, which Wikipedia calls the Categorical
%distribution.}
...
sets of documents \cite{blei2003latent}.  The generative model is as follows: %Simplifying slightly, the model is defined by the following steps:
\begin{itemize}
  \itemsep 5pt
...
multinomial probabilities for $w_{mn}$. % (See the paper
%``Latent Dirichlet allocation'' on the course website for more details if
%needed).
...
%\item Derive a mean-field algorithm for inference in the LDA model by
%minimizing the KL-divergence $D(q_{\gamma_m,\phi_m}(\theta, {\bf z}) || p(\theta, {\bf z} \mid {\bf w}))$ with respect to the variational parameters $\phi$ and $\gamma$, where $q_{\gamma, \phi}(\theta, {\bf z}) = q_\gamma(\theta) \prod_n q_{\phi_n}(z_n)$, 
%$q_\gamma(\theta)$ is a  Dirichlet, and the $q_{\phi_n}(z_n)$ are Multinomial.

%For this question only, it is permissible to consult external
%resources -- such as the LDA paper \cite{blei2003latent} -- to help
%you figure out the derivation (please cite any sources you used).  In particular, you will probably want to use the fact that:
%\[ \mbox{E}_{q_\gamma}\left[\log\theta_i\right] = \Psi(\gamma_i) - \Psi(\sum_{j=1}^k \gamma_j) \]
...
%\item Which algorithm converges fastest?  Do all algorithms return an accurate estimate of $E[\theta_m]$ when run for a sufficiently long time? Explain your answers.
